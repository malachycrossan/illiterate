...
// Zero test:  p.mag2() < EPS*EPS  (avoids sqrt)
// Parallel:   fabs(a.cross(b)) < EPS
// Same dir:   a.cross(b) ~= 0 && a.dot(b) > 0
...
// isect -- result of any intersection query
// count = 0 (miss), 1 (tangent/touch), 2 (two points)
// loop: for (int i = 0; i < h.count; i++) use(h.pt[i]);
...
// Implicit form: ax + by = c where a=-dir.y, b=dir.x, c=a*ox+b*oy
// Parametric form: origin + t*dir  (t=0 is origin, t=1 is the second point)
//
// at_param(t) -- point at parameter t
// at_dist(d)  -- point at distance d from origin along the line
...
// line x line using Cramer's rule.
// The determinant of the system is dir_a.cross(dir_b).
// Returns empty when parallel (det ~= 0) or coincident.
...
struct seg {
    ...
    // closest point on segment to p, clamped to endpoints
    P closest(P p) const {
        P ab = b-a;
        double len2 = ab.mag2();
...
struct circle {
    ...
    // area of a sector with central angle in radians: (angle/2) * r^2
    double sector_area(double angle_rad) const {
        return 0.5 * radius * radius * angle_rad;
    }

    // area of the triangle formed by the center and chord: 0.5 * r^2 * sin(angle)
    double triangle_area(double angle_rad) const {
        return 0.5 * radius * radius * sin(angle_rad);
    }

    // area of circular segment (sector minus triangle)
...
// poly -- polygon with vertices in order. CCW = positive signed area.
...
struct poly {
    ...
    // __gcd is a GCC built-in (equivalent to std::gcd from <numeric> in C++17)
    ...
    // ray casting -- works for any simple polygon regardless of winding
    ...
    // checks all consecutive edge cross products have the same sign
...
// Andrew's monotone chain, O(n log n). Returns CCW polygon.
// set keep_collinear=true to include collinear boundary points
...
// closest pair of points, O(n log n). input must have >= 2 points.
