// polygon.cpp
// Tests: poly area, signed_area, centroid, contains, is_convex,
//        boundary_points, interior_points (Pick's theorem), convex_hull
...
double angle(P a, P b) { return acos(max(-1.0, min(1.0, a.dot(b) / a.mag() / b.mag()))); }
...
int main() {
    ...
    // Each test case:
    //   Line 1: "AREA" | "CONTAINS" | "CONVEX" | "CENTROID" | "PICKS" | "HULL"
    //   Line 2: n (number of polygon vertices)
    //   Lines 3..n+2: x y (polygon vertices)
    //   For CONTAINS: after polygon, one query point "qx qy"
    //   For HULL: polygon is just a set of input points
    ...
    while (T--) {
        ...
        else if (strcmp(cmd,"SIGNED_AREA")==0) {
            poly pg(pts);
            printf("%.6f\n", pg.signed_area() + EPS);
        }
        else if (strcmp(cmd,"CONTAINS")==0) {
            double qx,qy; scanf("%lf %lf",&qx,&qy);
            poly pg(pts);
            int r = pg.contains(P(qx,qy));
            if (r==1) printf("INSIDE\n");
            else if (r==0) printf("BOUNDARY\n");
            else printf("OUTSIDE\n");
        }
        else if (strcmp(cmd,"CONVEX")==0) {
            poly pg(pts);
            printf("%s\n", pg.is_convex() ? "YES" : "NO");
        }
        else if (strcmp(cmd,"CENTROID")==0) {
            poly pg(pts);
            P c = pg.centroid();
            printf("%.6f %.6f\n", c.x + EPS, c.y + EPS);
        }
        else if (strcmp(cmd,"PICKS")==0) {
            poly pg(pts);
            printf("%lld %lld\n", pg.boundary_points(), pg.interior_points());
        }
        else if (strcmp(cmd,"HULL")==0) {
            poly h = convex_hull(pts);
            printf("%d\n", h.size());
            for (int i=0;i<h.size();i++)
                printf("%.6f %.6f\n", h.v[i].x + EPS, h.v[i].y + EPS);
        }
    }
}
